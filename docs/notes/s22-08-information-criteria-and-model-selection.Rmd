...
One method for choosing among a set of candidate models is to pick based on the residuals. In this method, we adopt the model for which the data best meet the assumptions, and employing the principle of parsimony--- adopting the more parsimonious model when there are multiple models that produce equally "good" residuals. Additionally, we can use the likelihood ratio test (LRT) to help make this deicsion. This method provides additional empirical evidence about which model is supported when the candidate models are nested.
...
Based on the AIC values, the candidate model with the most empirical evidence is the model with main effects of median SAT score and educatinal sector (Model 2); it has the lowest AIC. 
...
data.frame(
  ...
  kable(
    ...
    booktabs = TRUE,
    escape = FALSE,
    col.names = c("Model", "k", "AICc", "$\\Delta$AICc", "Rel($\\mathcal{L}$)"),
    caption = "Models rank-ordered by the amount of empirical support as measured by the AICc. The sample size used to fit each model was $n=33$.",
    digits = 1,
    table.attr = "style='width:50%;'"
    ) %>%
...
In all three models we are using the same data set and outcome. However, the assumptions only seem reasonably tenable for the second and third models (and maybe not even the third). That means that we should not really include Model 1 in our candidate set of models/working hypotheses. Changing the set of candidate models/working hypotheses we produce the following table of model evidence.
