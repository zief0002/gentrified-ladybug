...
```{r fig.cap='Proportion of students who obtain a degree conditional on ACT score. Size of the dot is proportional to sample size. The loess smoother is based on the raw data.'}
...
How does this work in a regression? There, we are relating the *predicted values*, that is, the $\beta_0 + \beta_1(X_i)$ values, to the probability of *Y*. So we need to apply the "S"-shaped logistic transformation to these values:
...
Let's re-examine the plot of the "S"-shaped example data, which showed the relationship between a predictor *X* and outcome *Y*, where *Y* was on the probability scale; akin to our ACT scores versus the probability of obtaining a degree. How does transforming *Y* to odds or log-odds change the relationship? 
...
```{r fig.height=4, fig.width=12, out.width="90%", fig.cap='Plot showing the relationship between the probability of *Y* versus *X* (LEFT), the odds of *Y* versus *X* (CENTER), and the log-odds of *Y* versus *X* (RIGHT).', class.source = 'fold-hide'}
...
```{r fig.cap='Predicted odds of obtaining a degree as a function of ACT score.'}
...
```{r fig.cap='Predicted probability of obtaining a degree as a function of ACT score.'}
# Plot the fitted equation
...
There are two ways to determine whether the ACT is a statistically significant predictor of the propbability of obtaining a degree. The first is to examine the *p*-value associated with the ACT predictor in the fitted model (classical framework). Since that is the only predictor included above-and-beyond the intercept, the *p*-value associated with it indicates whether the ACT predictor is statistically relevant.
...
When we fitted linear regression models, we computed an $R^2$ value for the model. This value quantified the amount of variation in the outcome that was explainable by the predictors included in the model. To compute this, we computed:
...
```{r fig.cap="Binned residuals versus the binned fitted values for the model that includes the linear effect of ACT to predict the log-odds of obtaining a degree."}
...
data.frame(
  ...
  kable(
    caption = "2x2 table of counts for degree obtainment by first generation status.",
    format = "html",
    escape = FALSE,
    align = c("l", "c", "c", "c"),
    digits = c(NA, 0, 0, 0),
    col.names = c("First Generation Status", "No", "Yes", "Total"),
    table.attr = "style='width:60%;'"
  ) %>%
  kable_classic() %>%
  add_header_above(c(" ", "Obtained Degree" = 2, "")) %>%
  add_indent(c(1, 2)) %>%
  column_spec(2:3, bold = c(FALSE, FALSE, TRUE), color = c("black", "black", "white"), background = c("white", "white", "#009E73")) %>%
  #row_spec(3, bold = , background = c("white", "#009E73", "#009E73")) %>%
  column_spec(4, bold = TRUE, color = "white", background = c("#CC79A7", "#CC79A7", "#0072B2"))
...
:::fyi
You need to select the the denominator to compute your proportions from based on your research questions. You also need to make sure that if you are using R (or another software) to compute proportions that the proportions you want are being outputted.
:::
...
```{r fig.cap='Side-by-side barplot showing the proportion of first generation and non-first generation students in the sample who obtained and did not obtain a degree.'}
...
```{r fig.cap="Binned residuals versus the binned fitted values for the model that includes the effect of first generation status to predict the log-odds of obtaining a degree."}
# Obtain average fitted values and average residuals
