...
In this set of notes, you will learn expand our knowledge of the logistic regression model to include non-linear predictor effects, covariates, and interaction terms. We will use data from the file *graduation.csv* (see the [data codebook](http://zief0002.github.io/epsy-8252/codebooks/graduation.html)) to explore predictors of college graduation. 
...
```{r fig.cap='Log-odds of obtaining a degree conditional on ACT score. Size of the dot is proportional to sample size. The loess smoother is also weighted based on the sample size.'}
...
```{r fig.show='hold', out.width="45%", fig.cap="Binned residual plots for two models that include non-linear effects of ACT. The quadratic model (LEFT) and the log-linear model (RIGHT)."}
...
The binned resdiuals show slightly better fit to the log-linear model than to the quadratic model. We also examine the empirical evidence via information criteria.
...
# Main Effects Models: Adding Covariates
...
While it is not germain to the research question, if you wanted to interpret the effect of ACT, you could plot the fitted model. As always with main effects models, we find the fitted equation for first genertation and non-first generation students, and plot the two curves separately using multiple `geom_function()` layers.
...
```{r fig.cap='Predicted odds of obtaining a degree as a function of ACT score first generation (solid, red line) and non-first generation (dashed, blue line) students.'}
# Plot the fitted equation
ggplot(data = grad, aes(x = act, y = got_degree)) +
  geom_point(alpha = 0) +
...
```{r fig.width=8, fig.height=6, fig.cap='Predicted probability of obtaining a degree as a function of ACT score for first generation (solid, red line) and non-first generation (dashed, blue line) students.'}
...
Next, we include scholarship amount into the model. An examination of the empirical log-odds of obtaining a degree versus scholarship amount also suggested a curvilinear relationship. We adopt a log-linear model, again adding 1 prior to transfomring using the logarithm since there are 0s in the variable.
